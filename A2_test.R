...
#Mapping customers' choice with the butter brand name
...
#Exercise 2
#=============
#I am going to use the conditional logit model with a constant
...
like_fun=function(param,CP)
{
  ...
  ut = mat.or.vec(ni,nj)
  beta    = param[10]
  #cons is the constant in conditional logit model
...
mix2_new$solution
mix2_new$objective
beta_r<-mix2_new$solution
beta_new<-beta_mix[-c(1,10)]

beta_new
L_r<-mix2_new$objective
L_f<-like_mix1(beta_new,new)

MTT<--2*(L_f-L_r)
norm<-sqrt(sum(beta_r^2))
